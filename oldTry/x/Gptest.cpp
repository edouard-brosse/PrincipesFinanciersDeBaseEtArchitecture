...
class OrderBook {
    ...
    void BuyInvoice(int dif, int cost){
        ...
        for (int size = 0; size < buyOrders.size(); size ++) {
            //std::cout << "___\nBoucle cost :: " << cost  <<" client Category: " << buyOrders[size].category << " | quantity: " << buyOrders[size].quantity << " | price: " << buyOrders[size].price << " unique ID " << buyOrders[size].ID << "\n___\n";
            if (buyOrders[size].category == 0 && buyOrders[size].price >= cost){
                if(dif >= buyOrders[size].quantity){
                    std::cout << "\n1 cost :: " << cost  <<" client ID: " << buyOrders[size].category << " | " << buyOrders[size].quantity << " | price: " << buyOrders[size].price << " unique ID " << buyOrders[size].ID << "\n";
                    std::cout << " Client ID " << buyOrders[size].clientID << " a acheter " << buyOrders[size].quantity << "\n";
                    //std::cout << "sdif :: " << dif - buyOrders[size].quantity << " dif: " << dif << " buyOrder quantity: " << buyOrders[size].quantity << "\n";
                    ...
                    std::cout << "\n2 cost :: " << cost  <<" client ID: " << buyOrders[size].category << " | " << buyOrders[size].quantity << " | price: " << buyOrders[size].price << " unique ID " << buyOrders[size].ID << "\n";
                    std::cout << "2222222 Client ID " << buyOrders[size].clientID << " a acheter " << cost << "\n";
                ...
                //std::cout << "Error cost :: " << cost  <<" client ID: " << buyOrders[size].category << " | " << buyOrders[size].quantity << " | price: " << buyOrders[size].price << " unique ID " << buyOrders[size].ID << "\n";
                //if(buyOrders[size].price == cost && buyOrders[size].category == 0){
                //    std::cout << "FInal Error cost :: " << cost << " buyOrderSize: " << buyOrders[size].price << " categori " << buyOrders[size].category << "\n";
                //} else if(buyOrders[size].price == cost){
                //    std::cout << "FInal Error Category cost :: " << cost << " buyOrderSize: " << buyOrders[size].price << " categori " << buyOrders[size].category << "\n";
                //}else{
                //    std::cout << "Error cost :: " << cost  <<" client ID: " << buyOrders[size].category << " | " << buyOrders[size].quantity << " | price: " << buyOrders[size].price << " unique ID " << buyOrders[size].ID << "\n";
                //}
            }
